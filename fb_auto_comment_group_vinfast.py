...
def generate_groq_comment(post_content: str) -> str:
    ...
    prompt = f"""
Bạn là một người dùng mạng xã hội bình thường, đọc bài post liên quan đến xe ô tô điện vinfast và đưa ra gợi ý về việc đã tìm thấy 1 sản phẩm decor xe vô cùng phù hợp và hiện đại với chiếc xe đang được nêu đến trong bài post.

Nhiệm vụ của bạn:
Đọc bài post, cố gắng trích xuất ra tên của xe vinfast điện đang được nhắc đến (ví dụ vf5, vf8, vf9, vf3, vf9, vfe34...), sau đó khen sản phẩm đó 1 câu hài hước và cuối cùng là dẫn dắt đến link sản phẩm với một câu nói theo ngôn ngữ xuề xòa, hài hước, tinh nghịch vui vẻ, 
giật gân (vừa hàng chính hãng lại còn vừa rẻ) và gắn link này vào cuối bình luận để khen dán decal vào trông chất lừ
.https://www.facebook.com/share/p/1HUGhY75yK/.

Noi dung bai viet:
"{post_content}"

YEU CAU QUAN TRONG: Viết những câu thật ngắn và tự nhiên như một người dùng thông thường chia sẽ, không viết dài dòng.
"""
...
def run_multi_group_commenting(
    ...
    max_comments: int = 1,
    image_dir: str = IMAGE_DIR,
    pause_between_groups: tuple = (20, 30),
):
    """
    ...
    for idx, group_url in enumerate(group_urls, start=1):
        ...
        try:
            ...
            success_count += 1
            results.append({"URL Nhóm": group_url, "Trạng thái": "Thành công", "Lý do": ""})
        except Exception as e:
            print(f"   WARN Loi nhom {idx}: {e}")
            fail_count += 1
            results.append({"URL Nhóm": group_url, "Trạng thái": "Thất bại", "Lý do": str(e)})
